...
internal class BooleanHandlerTest {
    ...
    @Test
    fun testEncryptTrue() {
        assertEquals("encrypted", handler.encrypt(true, contextMock, null))
        verify(encryptionServiceMock).encryptString(eq("true"), anyOrNull(), eq(null))
    ...
    fun testEncryptFalse() {
        assertEquals("encrypted", handler.encrypt(false, contextMock, null))
        verify(encryptionServiceMock).encryptString(eq("false"), anyOrNull(), eq(null))
        verify(contextMock, never()).encrypt(anyOrNull(), eq(null))
    }
